    ...
    <div class="flex flex-col gap-8 bg-white rounded-3xl shadow border border-gray-100 p-8 md:p-10">
        ...
        <form method="POST" action="{{ route('login.store') }}" class="flex flex-col gap-6">
            ...
            <div class="space-y-2">
                <div class="flex items-center justify-between ml-1">
                    <label for="password" class="block text-[10px] font-black uppercase tracking-[0.2em] text-gray-400">
                        {{ __('Password') }}
                    </label>
                    @if (Route::has('password.request'))
                        <a href="{{ route('password.request') }}"
                            class="text-[10px] font-black uppercase tracking-widest text-primary-600 hover:text-primary-700 transition"
                            wire:navigate>
                            {{ __('Forgot password?') }}
                        </a>
                    @endif
            ...
            <div class="flex items-center ml-1">
                <input type="checkbox" id="remember" name="remember"
                    class="w-4 h-4 text-primary-600 border-gray-200 rounded focus:ring-primary-600 cursor-pointer">
                <label for="remember" class="ml-3 text-xs font-bold text-gray-500 cursor-pointer select-none">
        ...
        @if (Route::has('register'))
            <div class="text-[10px] text-center font-black uppercase tracking-[0.2em] text-gray-400">
                <span>{{ __('New to Scholarpeep?') }}</span>
                <a href="{{ route('register') }}" class="text-primary-600 hover:text-primary-700 transition ml-1"
                    wire:navigate>
